...
module.exports = function (app, songsRepository) {
    app.get("/api/v1.0/songs", function (req, res) {
        ...
        songsRepository.getSongs(filter, options).then(songs => {
            ...
            res.status(500);
            res.json({error: "Se ha producido un error alrecuperar las canciones."})
        });
    });
    ...
    app.post('/api/v1.0/songs', function (req, res) {
        try {
            let song = {title: req.body.title, kind: req.body.kind, price: req.body.price, author: req.session.user}// Validar aquí:título, género, precio y autor.
